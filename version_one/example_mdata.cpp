...
int main()
{
	...
	vector<int> mlength={80,90,100,110,120,130,140,150};

	ofstream fout;
	fout.open("mdata.txt",ofstream::out);
	fout.close();

	for(auto it=mlength.begin();it!=mlength.end();it++)
	{
		parameters p(64,*it,100);
		cout<<endl<<endl;
		p.PrintParameters();
		...
		fout.open("mdata.txt",ofstream::app);
		fout<<"mdata = "<<*it<<endl;
